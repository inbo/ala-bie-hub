    ...
    function(data) {
      for (var i = 0; i < data.length; i++) {
        ...
        if (specieslist.list.isBIE) {
          let collapsiblePanelID =
            "specieslist-collapsible-panel-" +
            i +
            "-" +
          ...
          $description.attr(
            "id",
            "specieslist-block-" + i + "-" + specieslist.dataResourceUid,
          );
          $description.addClass("species-list-block");
          $description
            .find(".title > a")
            .attr("href", "#" + collapsiblePanelID);
          $description
            .find(".title > a")
            .attr("aria-controls", collapsiblePanelID);
          ...
          if (specieslist.kvpValues.length > 0) {
            ...
            $.each(specieslist.kvpValues, function(idx, kvpValue) {
              ...
              content +=
                "<tr><td>" +
                (kvpValue.key + "</td><td>" + value + "</td></tr>");
            });
            content += "</table>";
            ...
            $description
              .find(".content")
              .html("A species list provided by " + specieslist.list.listName);
          }
...
function addAlerts() {
  ...
  $("#alertsButton").click(function(e) {
    ...
    var url =
      SHOW_CONF.alertsUrl + "/webservice/createBiocacheNewRecordsAlert?";
    ...
    url +=
      "&webserviceQuery=%2Foccurrences%2Fsearch" +
      encodeURIComponent(searchString);
    url +=
      "&uiQuery=%2Foccurrences%2Fsearch" + encodeURIComponent(searchString);
...
function loadMap() {
  ...
  SHOW_CONF.map = L.map("leafletMap", {
    center: [
      SHOW_CONF.defaultDecimalLatitude,
      SHOW_CONF.defaultDecimalLongitude,
    ],
    ...
    function(data) {
      if (data) {
        if (data.totalRecords > 0) {
          ...
          $("#recordBreakdowns").html(
            "<h3>" + jQuery.i18n.prop("no.records.found") + "</h3>",
          );
        }
      }
    },
...
function fitMapToBounds() {
  ...
  $.getJSON(jsonUrl, function(data) {
    if (data.length == 4 && data[0] != 0 && data[1] != 0) {
      ...
      if (
        !mapBounds.contains(dataBounds) &&
        !mapBounds.intersects(dataBounds)
      ) {
    ...
    .done(function(data) {
      ...
      if (data.error) {
        ...
        $.each(data.categorical_traits, function(idx, traitValue) {
          ...
          tableRow +=
            traitValue.trait_values + "</td><td class='centered-cell'>";
          tableRow +=
            "<a target='_blank' href=" +
            traitValue.definition +
        ...
        $.each(data.numeric_traits, function(idx, traitValue) {
          ...
          tableRow +=
            (traitValue.min || " - ") + "</td><td class='centered-cell'>";
          tableRow +=
            (traitValue.mean || " - ") + "</td><td class='centered-cell'>";
          tableRow +=
            (traitValue.max || " - ") + "</td><td class='centered-cell'>";
        ...
        $("#traitsRecords").html(
          "<h3>" + jQuery.i18n.prop("no.traits.found") + "</h3>",
        );
        $("#download-button-area").hide();
        $("#traitsRecords .panel-footer").hide();
      }
    ...
    .done(function(data) {
      if (
        ...
        let htmlContent =
          "<span> There are " +
          data[0]["summary"] +
          " traits available for <span class='scientific-name rank-" +
          SHOW_CONF.rankString +
          "'><span class='name'>" +
          data[0]["taxon"] +
          "</span></span>  with data for " +
          data[0]["AusTraits"] +
          " further traits in the <a  target='_blank' href='" +
          SHOW_CONF.ausTraitsHomeUrl +
          "'>AusTraits</a> database. These are accessible via the download CSV button or alternatively the entire database can be accessed at  <a target='_blank' href='" +
          SHOW_CONF.ausTraitsSourceUrl +
          "'>" +
          SHOW_CONF.ausTraitsSourceUrl +
          "</a>. </span>";
...
function loadDataProviders() {
  ...
  $.getJSON(url, function(data) {
    if (data.totalRecords > 0) {
      ...
      if (
        data.facetResults[0].fieldResult[datasetCount - 1].label == "Unknown"
      ) {
      ...
      $.post({
        ...
        success: function(dataList) {
          $.each(dataList, function(idx, d) {
            ...
            var tableRow =
              "<td><a href='" +
              dataResourceUrl +
              "'><span class='data-provider-name'>" +
              facetMap[collectoryData.uid].label +
              "</span></a>";
            ...
            tableRow +=
              ...
              facetMap[collectoryData.uid].count +
              "</span></a></td>";
        ...
        $.each(data.facetResults[0].fieldResult, function(idx, facetValue) {
          if (facetValue.count > 0) {
            ...
            var dataResourceUrl =
              SHOW_CONF.collectoryUrl + "/public/show/" + uid;
            var tableRow =
              "<td><a href='" +
              dataResourceUrl +
              "'><span class='data-provider-name'>" +
    ...
    if (data.total > 0) {
      ...
      $.each(data.profiles, function(index, profile) {
        ...
        if (profile.mainVideo) {
          ...
          panel
            .find(".video-attribution")
            .append(profile.mainVideo.attribution);
        ...
        if (profile.mainAudio) {
          ...
          panel
            .find(".audio-attribution")
            .append(profile.mainAudio.attribution);
...
function renderPluginTab(speciesList, tabMetadata) {
  ...
  var sectionHtml =
    "<section class='tab-pane fade' id='" + tabMetadata.tab + "'></section>";
...
function showWikipediaData(data, testPage, targetName) {
  ...
  node.each(function(idx, item) {
    ...
    if (item.tagName == "SECTION") {
      ...
      var title;
      if (
        item.childNodes[0].tagName &&
        item.childNodes[0].tagName.match(/H[0-9]/)
      ) {
      ...
      $description
        ...
        .html(
          item.innerHTML.replaceAll(
            'href="./',
            'href="' + SHOW_CONF.wikipediaWebportalUrl,
          ),
        );
      ...
      var sourceHtml =
        ...
        "' target='wikipedia'>Wikipedia</a>&nbsp;" +
        jQuery.i18n.prop("wikipedia.licence.comment");
      $description.find(".sourceText").html(sourceHtml);

      // hide unused properties of this description
      var rights =
        "<a href='https://creativecommons.org/licenses/by-sa/4.0/'>" +
        jQuery.i18n.prop("wikipedia.licence.label") +
        "</a>";
  ...
  $.ajax({ url: SHOW_CONF.soundUrl })
    .done(function(data) {
      if (data.sounds) {
        ...
        soundsDiv +=
          '<a href="' +
          SHOW_CONF.biocacheUrl +
          "/occurrence/" +
          data.raw.rowKey +
          '">View more details of this audio</a></p>';
        soundsDiv += "</div></div>";
      ...
      $.ajax({
        ...
        success: function(data) {
          var record = {
            ...
            largeImageUrl:
              SHOW_CONF.imageServiceBaseUrl + "/image/" + imageId + "/large",
          };
...
function addOverviewImages(imagesArray, countPreferredImages) {
  ...
  for (j = 0; j < imagesArray.length && addedImages < 5; j++) {
    // load images
    if (
      imagesArray.length > j &&
      (SHOW_CONF.preferredImageId ||
        SHOW_CONF.preferredImageId.indexOf(imagesArray[j].image) < 0) &&
...
function addOverviewImage(overviewImageRecord) {
  ...
  $mainOverviewImage
    .parent()
    .attr("data-title", getImageTitleFromOccurrence(overviewImageRecord));
  $mainOverviewImage
    .parent()
    .attr("data-footer", getImageFooterFromOccurrence(overviewImageRecord));
  $mainOverviewImage.parent().attr("data-image-id", overviewImageRecord.image);
  $mainOverviewImage
    .parent()
    .attr(
      "data-record-url",
      SHOW_CONF.biocacheUrl + "/occurrences/" + overviewImageRecord.uuid,
    );
  ...
  $(".mainOverviewImageInfo").html(
    getImageTitleFromOccurrence(overviewImageRecord),
  );
}
...
function addOverviewThumb(record, i) {
  if (i < 4) {
    ...
    $("#more-photo-thumb-link").attr(
      "style",
      "background-image:url(" + record.smallImageUrl + ")",
    );
  }
}
...
function generateOverviewThumb(occurrence, id) {
  ...
  $taxonSummaryThumb.attr(
    "style",
    "background-image:url(" + occurrence.smallImageUrl + ")",
  );
  $taxonSummaryThumbLink.attr(
    "data-title",
    getImageTitleFromOccurrence(occurrence),
  );
  $taxonSummaryThumbLink.attr(
    "data-footer",
    getImageFooterFromOccurrence(occurrence),
  );
  $taxonSummaryThumbLink.attr("href", occurrence.largeImageUrl);
  $taxonSummaryThumbLink.attr("data-image-id", occurrence.image);
  $taxonSummaryThumbLink.attr(
    "data-record-url",
    SHOW_CONF.biocacheUrl + "/occurrences/" + occurrence.uuid,
  );
  $taxonSummaryThumb
    .find(".hero-button")
    .attr(
...
function heroImage(imageId) {
  ...
  var order = parseInt(
    prompt(jQuery.i18n.prop("confirm.hero.image"), originalOrder),
  );
  ...
  if (order != originalOrder && !isNaN(order)) {
    ...
    var url =
      "/externalSite/setImages?guid=" +
      encodeURIComponent(SHOW_CONF.guid) +
      "&prefer=" +
      encodeURIComponent(newImageIds.join(",")) +
      "&name=" +
      encodeURIComponent(SHOW_CONF.scientificName) +
      "&hide=" +
...
function loadGalleryType(category, start) {
  var imageCategoryParams = {
    ...
    other:
      '&fq=-type_status:*&fq=-basis_of_record:PreservedSpecimen&fq=-identification_qualifier_s:"Uncertain"&fq=spatiallyValid:true&fq=-user_assertions:50001&fq=-user_assertions:50005',
    uncertain:
      '&fq=-type_status:*&fq=-basis_of_record:PreservedSpecimen&fq=identification_qualifier_s:"Uncertain"',
  };
  ...
  var url =
    ...
    "/occurrences/search?q=lsid:" +
    SHOW_CONF.guid +
    '&fq=multimedia:"Image"&pageSize=' +
    pageSize +
    "&facet=off&start=" +
    start +
    imageCategoryParams[category] +
    "&im=true" +
    SHOW_CONF.imageFilter;
    ...
    if (data && data.totalRecords > 0) {
      ...
      $.each(data.occurrences, function(i, el) {
        $.each(el.images, function(j, imageId) {
          ...
          $taxonThumb
            .find("img")
            .attr("src", el.smallImageUrl.replace(el.image, imageId));
          ...
          $taxonThumb
            ...
            .attr(
              "onclick",
              'event.stopImmediatePropagation(); heroImage("' + imageId + '");',
            );
          ...
          $taxonThumb.attr(
            "data-record-url",
            SHOW_CONF.biocacheUrl + "/occurrences/" + el.uuid,
          );
      ...
      if (data.totalRecords > start + pageSize) {
        ...
        var btn =
          '<div class="loadMore ' +
          category +
          '"><br><button class="btn btn-default" onCLick="loadGalleryType(\'' +
          category +
          "'," +
          (start + pageSize) +
          ');">Load more images <img src="' +
          spinnerLink +
          '" class="hide"/></button></div>';
  ...
  if (SHOW_CONF.taxonRankID < 7000) {
    briefHtml +=
      el.raw_scientificName === undefined
        ? el.scientificName
        : el.raw_scientificName; //raw scientific name can be null, e.g. if taxon GUIDS were submitted
  }
...
function getImageFooterFromOccurrence(el) {
  ...
  if (el.eventDate)
    detailHtml += br + "Date: " + moment(el.eventDate).format("YYYY-MM-DD");
  ...
  if (
    el.imageMetadata &&
    el.imageMetadata.length > 0 &&
    el.imageMetadata[0].rightsHolder != null
  ) {
  ...
  if (el.uuid) {
    detailHtml +=
      '<div class="recordLink"><a href="' +
      SHOW_CONF.biocacheUrl +
      "/occurrences/" +
      el.uuid +
      '">View details of this record</a>' +
      "<br><br>If this image is incorrectly<br>identified please flag an<br>issue on the <a href=" +
      SHOW_CONF.biocacheUrl +
      "/occurrences/" +
      el.uuid +
      ">record.<br></div>";
...
function showDistribution() {
  ...
  $("#dataResourceAreaName").text(
    distributionsIdx + 1 + ": " + distributions[distributionsIdx].name,
  );
